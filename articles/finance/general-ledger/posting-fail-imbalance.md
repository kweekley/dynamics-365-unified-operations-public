...
If the voucher has more than one transaction currency, each line of the voucher is translated to the accounting currency and rounded to two decimal places, and then the lines are summed to determine the total debits and total credits. To be considered balanced, the total debits and total credits must be balanced in the accounting currency. A penny rounding difference will never be created in the accounting currency when there is more than one transaction currency within the voucher.
...
If the voucher has more than one transaction currency, each line of the voucher is translated to the reporting currency and rounded to two decimal places, and then the lines are summed to determine the total debits and total credits. To be considered balanced, the total debits and total credits must be balanced in the reporting currency. A penny rounding difference will never be created in the reporting currency when there is more than one transaction currency within the voucher.
